...
(:test)
function testChooseToken_storedWins(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(chooseToken("stored-abc", "change-me"), "stored-abc");
    return true;
}

(:test)
function testChooseToken_fallsBackToProperty(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(chooseToken(null, "my-key"), "my-key");
    Test.assertEqual(chooseToken("", "my-key"), "my-key");
    return true;
}

(:test)
function testChooseToken_placeholderIsNotToken(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(chooseToken(null, "change-me"), null);
    return true;
}

(:test)
function testChooseToken_emptyEverywhereIsNull(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(chooseToken(null, ""), null);
    Test.assertEqual(chooseToken(null, null), null);
    return true;
}

// ---- oauthRequestParams / oauthResultKeys --------------------------------

(:test)
function testOauthRequestParams_shape(logger as Test.Logger) as Lang.Boolean {
    var p = oauthRequestParams("https://backend.example.com/oauth/callback");
    Test.assertEqual(p.get("response_type"), "code");
    Test.assertEqual(p.get("client_id"), "garmin-router-widget");
    Test.assertEqual(p.get("redirect_uri"), "https://backend.example.com/oauth/callback");
    return true;
}

(:test)
function testOauthResultKeys_mapsCode(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(oauthResultKeys().get("code"), "code");
    return true;
}
...
(:test)
function testParseOAuthCode_happyPath(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(parseOAuthCode({"code" => "AUTH-XYZ"}), "AUTH-XYZ");
    return true;
}

(:test)
function testParseOAuthCode_missingOrEmpty(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(parseOAuthCode({}), null);
    Test.assertEqual(parseOAuthCode({"code" => ""}), null);
    Test.assertEqual(parseOAuthCode(null), null);
    return true;
}
...
(:test)
function testParseTokenResponse_happyPath(logger as Test.Logger) as Lang.Boolean {
    var data = {"access_token" => "TOK-123", "token_type" => "bearer"};
    Test.assertEqual(parseTokenResponse(data), "TOK-123");
    return true;
}

(:test)
function testParseTokenResponse_missingOrWrongType(logger as Test.Logger) as Lang.Boolean {
    Test.assertEqual(parseTokenResponse({}), null);
    Test.assertEqual(parseTokenResponse({"access_token" => ""}), null);
    Test.assertEqual(parseTokenResponse(null), null);
    Test.assertEqual(parseTokenResponse("not-a-dict"), null);
    return true;
}
