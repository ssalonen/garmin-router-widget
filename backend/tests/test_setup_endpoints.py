...
configures it; TOKEN is supplied in each POST body.
"""
import re
...
def test_setup_disabled_without_setup_token(client, monkeypatch):
    ...
    assert client.post("/setup/login",
                       data={"email": "a@b.c", "password": "pw", "token": TOKEN}).status_code == 403


# ── GET /setup (serves the form; token entered on POST, not in the URL) ───────

def test_setup_page_serves_form_without_url_token(client):
    r = client.get("/setup")  # no ?token=
    assert r.status_code == 200
    assert "text/html" in r.headers["content-type"]
    assert 'name="token"' in r.text        # setup token is a form field
...
def test_login_bad_credentials_reshows_form(client, fake_auth):
    ...
    assert 'name="token"' in r.text          # form re-rendered for retry
...
def test_mfa_form_escapes_reflected_token(client, fake_auth, monkeypatch):
    """The token is carried into the MFA form as a hidden field; it must be
    HTML-escaped. Use an XSS payload as the configured token so the gate passes."""
    payload = '"><script>alert(1)</script>'
    monkeypatch.setenv("SETUP_TOKEN", payload)
    fake_auth.mfa = True
    r = client.post("/setup/login",
                    data={"email": "a@b.c", "password": "pw", "token": payload})
    assert r.status_code == 200
