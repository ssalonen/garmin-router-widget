...
was never implementable. The only device-side mechanism is a FIT download:
`makeWebRequest` with `:responseType => HTTP_RESPONSE_CONTENT_TYPE_FIT`, which
the OS parses and stores as device course content under `GARMIN/Courses/`,
reachable from Navigation > Courses and `PersistedContent`.

`GET /api/course/{id}?name=...` serves that file, generated by `backend/fit.py`
from the lat/lon pairs `GarminSession.get_course_points()` returns. Generating
it ourselves (rather than proxying a Garmin export) keeps control of the wire
format — point decimation, optional fields — and `python-garminconnect` has no
course export endpoint anyway.

### Record format and payload cost

Measured by the e2e suite on the edge530 simulator, each scenario starting from
an empty course store:

| records | bytes/point | accepted |
|---|---|---|
| lean (positions only) — **default** | 9.16 | yes |
| full (timestamp + position + distance) — `?lean=0` | 17.16 | yes |
| deliberately corrupted | — | **no, rejected** |

Lean is the default because it is also cheaper than the ASCII85 point payload
it replaced (10.00 B/pt), so moving to FIT made the wire smaller rather than
larger. `?lean=0` is the escape hatch if a physical device turns out to be
stricter than the simulator — it is a backend query parameter, so switching
needs no widget rebuild or re-sideload.

The corrupt case is a control, not a curiosity: if a damaged file were stored
too, the device would not be validating anything and the positive results would
prove nothing. It is asserted on every CI run for that reason.

### Two traps

- **HTTP 200 does not mean the FIT was accepted.** The code describes the
  transfer, not the parse; a rejected file reports 200 and is then silently
  discarded. Acceptance is observable only in the course store, which is what
  the e2e tests assert on.
- **`PersistedContent.getCourses()` can be stale.** In the corrupt-file
  scenario it returned a course whose file had already been removed from disk.

Together these mean the widget cannot confirm a save, so it does not claim to:
the end screen reads "Downloaded" and points at the Courses menu rather than
asserting the file is there. `PERSISTED_COURSE*` lines in the log are
diagnostics only.
